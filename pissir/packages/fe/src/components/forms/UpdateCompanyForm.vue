    ...
    const emitUpdateCompany = () => {
      ...
      schema
        ...
        .then((body) => {
          ...
          emit('updateCompany', props.companyID, body, loading);
        })
        .catch((e) => {
          loading.value = false;
          Object.keys(errors).forEach(key => delete errors[key]);
...
<template>
  <div class="d-flex flex-column gap-1">
    ...
    <a-button class="radius-1 mt-1" :loading="loading" @click="emitUpdateCompany">{{ t("form.update") }}</a-button>
  </div>
</template>